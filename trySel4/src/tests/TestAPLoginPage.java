...
public class TestAPLoginPage {
  ...
  public void testLoginPage(String email, String pswrd, String type) throws InterruptedException {
	...
	/* //Verify page title
	  ...
	  objMyAccount = objLoginPage.loginAsValidUser(email, pswrd);
	    ...
	    objLoginErrorPage = objLoginPage.loginAsInvalidUser(email, pswrd);
	    try{
	    	String text="There is 1 error";
	    	boolean flag=objLoginErrorPage.loginError().getText().contains(text);
  ...
  public void testSignUpEmail(String email, String type) throws InterruptedException {
	  ...
	  objSignUpPage = objLoginPage.signUpAsValidUser(email);
	    ...
	    try{
	    	...
	    	Reporter.log("Signup form found");
	    	WebElement signIn=objSignUpPage.getSignUpPageSignIn();
	    	signIn.click();
	    	driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
	    }
	    catch (AssertionError e){
			  Reporter.log("Error in Signup");
			  Reporter.log(e.getMessage());
		  }
	  }
	    
	    else{
	    objLoginErrorPage = objLoginPage.signUpAsInvalidUser(email);
	    try{
	    	Thread.sleep(3000);
	       WebElement msg =objLoginErrorPage.signUpError();
	  	   assertEquals(true, msg.isDisplayed());
	    	Reporter.log("Sign up error message displayed " +msg.getText());
	    }
	    catch (AssertionError e){
			  Reporter.log("Error while signing up");
			  Reporter.log(e.getMessage());
		  }
	    }
	  }
  ...
  @DataProvider
	public Object[][] SignUp() {
	  return new Object[][] {
	      new Object[] { "frank@boyle.com", "valid" },
	      new Object[] { "jimmy@carr.com", "valid" },
